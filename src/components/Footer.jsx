...
export function Footer(props) {
  ...
  return (
    <section className='grid grid-cols-2 md:grid-cols-3 gap-4 bg-primary text-light p-4 px-5 md:px-20 relative z-20'>
      <img src="/assets/img/logo.png" alt="logo" className='logo w-fit' />
        ...
        <span className='flex gap-3'>
          {
            references.socials.map((social, index) => {
              return <img onClick={() => { visitLink(social.link) }} key={index} className='object-contain h-[30px]' src={social.iconImg} alt={social.name} />
            })
          }
          <img src="/assets/img/Youtube.png" alt="youtube" className='object-contain h-[30px]' />
        </span>
      </div>
    </section>
  )
